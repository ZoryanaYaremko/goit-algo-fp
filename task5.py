...
def generate_color(step, total_steps):
    base_color = [0, 0, 255]  # Темний синій колір
    lightest_color = [135, 206, 250]  # Світло-блакитний
    
    # Проміжок для зміни кольору
    color_diff = [(lightest_color[i] - base_color[i]) / total_steps for i in range(3)]
    
    new_color = [int(base_color[i] + color_diff[i] * step) for i in range(3)]
